  ...
  describe("can only vote once", function () {
    it("revert because proposal does not exist ", async function () {
      const { dao, places, owner } = await loadFixture(deployPlaceAndDao);
       await expect(dao.VoteForProposal(13, 0, true)).to.be.revertedWith("Proposal does not exist");
    ...
    it("revert if voting for proposal 0 ", async function () {
      ...
      await expect(dao.VoteForProposal(0, 0, true)).to.be.revertedWith(
    ...
    it("revert if voting for unexisting proposal", async function () {
      ...
      await expect(dao.VoteForProposal(1, 4, true)).to.be.revertedWithoutReason();
    });
    ...
    it("vote against ", async function () {
      ...
      const result = await dao.VoteForProposal(1, 0, false);
      const tx = await result.wait();
      ...
      expect(event?.args[3]).to.be.equal(false);
      expect(event?.args[4]).to.be.equal(0);
      expect(event?.args[5]).to.be.equal(1);
    ...
    it("vote not enougth to be executed ", async function () {
      ...
      await dao.connect(account1).VoteForProposal(1, 0, true);
      ...
      await dao.connect(account3).VoteForProposal(1, 0, false);
      proposal = await dao.getProposalInfos(1, 0);

      expect(proposal.votesAgainst).to.be.equal(2);
      expect(proposal.votesFor).to.be.equal(1);
      expect(proposal.executed).to.be.equal(false);

      await dao.connect(account2).VoteForProposal(1, 0, true);
      ...
      await dao.connect(account4).VoteForProposal(1, 0, true);
